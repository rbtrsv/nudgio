# server/apps/nexotype_surreal/utils/genomics_utils.py
...
def predict_peptide_activities(sequence: str) -> List[str]:
    ...
    # Simple rule-based activity prediction based on amino acid patterns
    # Note: These are simplified heuristics
    
    # Antimicrobial peptide characteristics
    if sequence.count('K') + sequence.count('R') > len(sequence) * 0.25:
        activities.append('Antimicrobial')
    
    # Cell-penetrating properties
    if 'KKK' in sequence or 'RRR' in sequence or (sequence.count('W') >= 1 and len(sequence) < 15):
        activities.append('Cell penetrating')
    
    # Stability characteristics
    if 'P' in sequence and sequence.count('P') >= len(sequence) * 0.1:
        activities.append('Stable')
    
    # Enzyme inhibitor properties
    if ('C' in sequence and sequence.count('C') >= 2) or ('NPXY' in sequence):
        activities.append('Enzyme inhibitor')
    
    # Hormone-like properties
    if len(sequence) <= 15 and (sequence.startswith('Y') or sequence.endswith('F')):
        activities.append('Hormone-like')
    
    # If no activities predicted, add a default
    if not activities:
...
def estimate_synthesis_difficulty(sequence: str) -> int:
    """
    Estimate the difficulty of synthesizing a peptide on a scale of 1-10
    ...
    # Length factor - longer peptides are harder to synthesize
    if len(sequence) <= 5:
        difficulty += 1
    ...
    # Cysteine - disulfide bridges make synthesis complex
    if 'C' in sequence:
        cys_count = sequence.count('C')
        difficulty += min(cys_count, 3)
    
    # Hydrophobic stretches - can cause aggregation
    hydrophobic = ['V', 'I', 'L', 'F', 'W', 'M']
    hydrophobic_count = sum(sequence.count(aa) for aa in hydrophobic)
    if hydrophobic_count > len(sequence) * 0.5:
        difficulty += 1
    
    # Difficult couplings
    difficult_pairs = ['DP', 'DN', 'PG', 'PN']
...
def calculate_peptide_similarity(seq1: str, seq2: str) -> float:
    ...
    # Simple similarity based on matching amino acids
    # For more sophisticated comparison, algorithms like Smith-Waterman or BLOSUM matrices would be used
    ...
    # Account for length difference in the score
    length_factor = min_len / max_len
    position_match_factor = matches / min_len
    
    # Combine factors: 70% based on position matches, 30% on length similarity
    similarity = (0.7 * position_match_factor) + (0.3 * length_factor)
