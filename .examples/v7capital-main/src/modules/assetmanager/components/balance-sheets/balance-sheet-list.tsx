...
import { type BalanceSheet, type FinancialScenario } from '@/modules/assetmanager/schemas/balance-sheets.schemas';
...
export default function BalanceSheetList() {
  ...
  const [statementToDelete, setStatementToDelete] = useState<BalanceSheet | null>(null);
  ...
  // Filter balance sheets based on search term, company, and scenario filter
  const filteredStatements = balanceSheets.filter(statement => {
  ...
  const handleDelete = async (statement: BalanceSheet) => {
    if (statement.id) {
      // Note: Delete functionality needs to be added to the hook
      // const success = await deleteBalanceSheet(statement.id);
      // if (success) {
  ...
  const handleViewDetails = (id: number) => {
    router.push(`/dashboard/balance-sheets/${id}`);
  };
  ...
  const handleCreate = () => {
    router.push('/dashboard/balance-sheets/new');
  };
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Loading balance sheets...</CardDescription>
        </CardHeader>
  ...
  if (error) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Balance Sheets</CardTitle>
          <CardDescription>Error loading balance sheets</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="bg-destructive/10 p-4 rounded-md text-destructive">
            {error}
          </div>
          <Button onClick={() => fetchBalanceSheets()} className="mt-4">Try Again</Button>
        </CardContent>
      </Card>
    );
  }
        ...
        <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          ...
          <div className="shrink-0">
            <Button onClick={handleCreate} className="w-full sm:w-auto">
              <Plus className="mr-2 h-4 w-4" /> Add Balance Sheet
            </Button>
          </div>
        </div>
      </CardHeader>
          ...
          <Table className="text-sm min-w-[1200px]">
            <TableCaption className="text-xs text-muted-foreground">
              {filteredStatements.length === 0 
                ...
                : `Showing ${startIndex + 1} to ${Math.min(endIndex, filteredStatements.length)} of ${filteredStatements.length} balance sheets`}
            </TableCaption>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="font-semibold text-right">Total Assets</TableHead>
                <TableHead className="font-semibold text-right">Total Liabilities</TableHead>
                <TableHead className="font-semibold text-right">Total Equity</TableHead>
            ...
            <TableBody className="text-sm">
              {paginatedStatements.map((statement) => (
                <TableRow key={statement.id} className="hover:bg-muted/50">
                  ...
                  <TableCell className="font-mono text-xs">{statement.date ? formatDate(statement.date) : '-'}</TableCell>
                  <TableCell className="font-mono text-xs">{statement.date ? formatDate(statement.date) : '-'}</TableCell>
                  <TableCell>{getScenarioBadge(statement.scenario)}</TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.totalAssets ? formatCurrency(statement.totalAssets, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.totalLiabilities ? formatCurrency(statement.totalLiabilities, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.totalStakeholdersEquity ? formatCurrency(statement.totalStakeholdersEquity, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.cashEquivalents ? formatCurrency(statement.cashEquivalents, 'USD') : '-'}
                  </TableCell>
                ...
                <div className="flex items-center gap-2 text-xs">
                  {statement.totalAssets && (
                    <Badge variant="outline" className="text-xs font-mono">
                      Assets: {formatCurrency(statement.totalAssets, 'USD')}
                    </Badge>
                  )}
                  {statement.totalStakeholdersEquity && (
                    <Badge variant="secondary" className="text-xs font-mono">
                      Equity: {formatCurrency(statement.totalStakeholdersEquity, 'USD')}
                    </Badge>
                  )}
                </div>
              </div>
