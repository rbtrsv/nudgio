...
import { type IncomeStatement, type FinancialScenario } from '@/modules/assetmanager/schemas/income-statements.schemas';
...
export default function IncomeStatementList() {
  ...
  const [statementToDelete, setStatementToDelete] = useState<IncomeStatement | null>(null);
  ...
  // Filter income statements based on search term, company, and scenario filter
  ...
  const handleDelete = async (statement: IncomeStatement) => {
    if (statement.id) {
      // Note: Delete functionality needs to be added to the hook
      // const success = await deleteIncomeStatement(statement.id);
      // if (success) {
  ...
  const handleViewDetails = (id: number) => {
    router.push(`/dashboard/income-statements/${id}`);
  };

  const handleEdit = (id: number) => {
    router.push(`/dashboard/income-statements/${id}/edit`);
  };

  const handleCreate = () => {
    router.push('/dashboard/income-statements/new');
  };
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Loading income statements...</CardDescription>
        </CardHeader>
  ...
  if (error) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Income Statements</CardTitle>
          <CardDescription>Error loading income statements</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="bg-destructive/10 p-4 rounded-md text-destructive">
            {error}
          </div>
          <Button onClick={() => fetchIncomeStatements()} className="mt-4">Try Again</Button>
        </CardContent>
      </Card>
    );
  }
        ...
        <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          <div className="min-w-0 flex-1">
            ...
            <CardDescription className="mt-1">
              Manage your income statements
            </CardDescription>
          </div>
          <div className="shrink-0">
            <Button onClick={handleCreate} className="w-full sm:w-auto">
              <Plus className="mr-2 h-4 w-4" /> Add Income Statement
            </Button>
          </div>
        </div>
      </CardHeader>
          ...
          <Table className="text-sm min-w-[1200px]">
            <TableCaption className="text-xs text-muted-foreground">
              {filteredStatements.length === 0 
                ...
                : `Showing ${startIndex + 1} to ${Math.min(endIndex, filteredStatements.length)} of ${filteredStatements.length} income statements`}
            </TableCaption>
            <TableHeader>
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="font-semibold text-right">Operating Income</TableHead>
                ...
                <TableHead className="font-semibold text-right">EPS Basic</TableHead>
              ...
              {paginatedStatements.map((statement) => (
                <TableRow key={statement.id} className="hover:bg-muted/50">
                  ...
                  <TableCell className="text-right font-mono">
                    {statement.revenue ? formatCurrency(statement.revenue, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.grossProfit ? formatCurrency(statement.grossProfit, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.operatingIncome ? formatCurrency(statement.operatingIncome, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.ebitda ? formatCurrency(statement.ebitda, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.netIncome ? formatCurrency(statement.netIncome, 'USD') : '-'}
                  </TableCell>
                  <TableCell className="text-right font-mono">
                    {statement.epsBasic ? Number(statement.epsBasic).toFixed(2) : '-'}
                  </TableCell>
                ...
                <div className="flex items-center gap-2 text-xs">
                  {statement.revenue && (
                    <Badge variant="outline" className="text-xs font-mono">
                      Revenue: {formatCurrency(statement.revenue, 'USD')}
                    </Badge>
                  )}
                  {statement.netIncome && (
                    <Badge variant="secondary" className="text-xs font-mono">
                      Net: {formatCurrency(statement.netIncome, 'USD')}
                    </Badge>
                  )}
                </div>
              </div>
