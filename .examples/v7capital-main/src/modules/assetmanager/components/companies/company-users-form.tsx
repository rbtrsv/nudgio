...
import { Save, UserPlus, Pencil, Trash2, Search } from 'lucide-react';
import { type CompanyUserWithProfile, type CompanyRole } from '../../schemas/companies.schemas';
...
export default function CompanyUsers({ companyId, userProfileId }: CompanyUsersProps) {
  ...
  const [editingUser, setEditingUser] = useState<CompanyUserWithProfile | null>(null);
  ...
  }, [companyId, fetchCompany, fetchCompanyUsers]);
  ...
  const availableUserProfiles = allUserProfiles.filter(
    profile => !companyUsers.some(user => user.userProfileId === profile.id)
  );
  ...
  const handleRemoveUser = async (user: CompanyUserWithProfile) => {
    if (user.userProfileId && user.companyId) {
      const success = await removeUserFromCompany(user.userProfileId, user.companyId);
      if (success) {
  ...
  const addUserForm = useForm({
    ...
    onSubmit: async ({ value }) => {
      const success = await addUserToCompany(
        value.userProfileId, 
        companyId, 
        value.role
      );
  ...
  const editUserForm = useForm({
    defaultValues: {
      role: (editingUser?.role || 'VIEWER') as CompanyRole,
    },
    onSubmit: async ({ value }) => {
  ...
  useEffect(() => {
    if (userProfileId && companyUsers.length > 0) {
      const user = companyUsers.find(u => u.userProfileId === userProfileId);
      if (user) {
        setEditingUser(user);
        editUserForm.setFieldValue('role', user.role);
      }
    }
  }, [userProfileId, companyUsers, editUserForm]);
      ...
      <>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {selectedCompany?.name ? ` in ${selectedCompany.name}` : ''}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
                  ...
                  {(field) => (
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value: CompanyRole) => field.handleChange(value)}
                      >
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Manage Users for {selectedCompany.name}</CardTitle>
        ...
        {isAddingUser ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Add a new user to this company</CardDescription>
            </CardHeader>
            {availableUserProfiles.length === 0 ? (
              <CardContent>
                <div className="bg-muted p-4 rounded-md text-center">
                  <p className="text-muted-foreground">All users have already been added to this company.</p>
              ...
              <CardFooter className="flex justify-between pt-6">
                <Button variant="outline" onClick={() => setIsAddingUser(false)}>
                  Cancel
                </Button>                  <addUserForm.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
                    {([canSubmit, isSubmitting]) => (
          ...
          <Table>
            <TableCaption>
              {companyUsers.length === 0 
                ...
                : `Showing ${companyUsers.length} users for this company`}
            </TableCaption>
            <TableHeader>
              <TableRow>
