...
import { CreateSmallMoleculeSchema } from '@/modules/nexotype/schemas/asset/small-molecule.schemas';
...
export default function CreateSmallMoleculePage() {
  const router = useRouter();
  const { createSmallMolecule, error: storeError } = useSmallMolecules();
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        const payload = {
          ...
          smiles: value.smiles,
          inchi_key: value.inchi_key || undefined,
        };

        // Validate with Zod
        const validation = CreateSmallMoleculeSchema.safeParse(payload);

        if (!validation.success) {
          return;
        }

        const success = await createSmallMolecule(payload);
    ...
    <div className="max-w-2xl mx-auto space-y-6 px-4 sm:px-0">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Add a new small molecule (NME) to the asset registry
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter the details for your new small molecule
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
          ...
          >
            <div className="space-y-4">
                ...
                {(field) => (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Unique identifier for this small molecule
                    </p>
              ...
              >
                {(field) => (
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Common or trade name of the small molecule
                    </p>
                    {field.state.meta.errors.length > 0 && (
                      <p className="text-sm text-destructive">
                        {field.state.meta.errors[0]}
              ...
              >
                {(field) => (
                  <div className="space-y-2">
                    <Label htmlFor={field.name}>SMILES *</Label>
                    <Textarea
                      id={field.name}
                      name={field.name}
                      value={field.state.value}
                ...
                validators={{
                  onChange: ({ value }) => {
                    if (value && value.length > 27) {
                      return 'InChI Key must be 27 characters or less';
                    }
                    return undefined;
                  },
                }}
              >
                {(field) => (
                  <div className="space-y-2">
                    <Label htmlFor={field.name}>InChI Key</Label>
                    <Input
                      id={field.name}
                      name={field.name}
                      value={field.state.value}
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      International Chemical Identifier hash key
                    </p>
                    {field.state.meta.errors.length > 0 && (
                      <p className="text-sm text-destructive">
                        {field.state.meta.errors[0]}
              ...
              <div className="flex flex-col-reverse gap-3 pt-4 sm:flex-row">
                ...
                <Link href="/small-molecules" className="flex-1">
                  <Button
                    type="button"
                    variant="outline"
                    disabled={form.state.isSubmitting}
